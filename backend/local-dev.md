...
1. Build and deploy a build of the stack on AWS or make an S3 bucket somewhere that SAM can access locally.
...
1. Build the project with `npm run build:local`, this will do the following three things:
   1. Build [poll-sites.ts](src/functions/process/poll-sites.ts) lambda and copy required files for Docker.
   2. Generate the CDK template.
   3. Build with SAM, which will build the Docker image.
2. Run a lambda function with SAM: `sam local invoke ProcessSites -n env.json -t cdk.out/WebsiteAlerter.template.json`. Sample Lambda events exist in the [sample](sample) folder.
